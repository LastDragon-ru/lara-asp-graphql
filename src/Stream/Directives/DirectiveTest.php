...
use LastDragon_ru\LaraASP\GraphQL\Package\Data\Queries\Query;
use LastDragon_ru\LaraASP\GraphQL\Package\Data\Types\CustomType;
use LastDragon_ru\LaraASP\GraphQL\Package\Data\Types\CustomType\Field;
use LastDragon_ru\LaraASP\GraphQL\Package\Models\Car;
use LastDragon_ru\LaraASP\GraphQL\Package\Models\CarEngine;
...
final class DirectiveTest extends TestCase {
    ...
    public function testManipulateFieldDefinitionBuilderUnknown(): void {
        ...
        $directive
            ...
            ->value(
                $this->app()->make(BuilderInfoDetector::class),
            );
        $directive
            ->shouldUseProperty('streamType')
            ->value(
                $this->app()->make(StreamType::class),
            );
        $directive
            ->shouldUseProperty('manipulatorFactory')
            ->value(
                $this->app()->make(ManipulatorFactory::class),
            );
        $directive
            ->shouldReceive('getBuilderInfo')
            ->once()
            ->andReturn(null);

        $this->override(Directive::class, $directive);
    ...
    public function testGetBuilderInfo(
        ...
        if ($builder !== null) {
            $directive
                ->shouldUseProperty('streamFactory')
                ->value(
                    Mockery::mock(StreamFactory::class)
    ...
    public function testGetResolverQuery(): void {
        ...
        $directive->makePartial();
        $directive
            ->shouldUseProperty('provider')
            ->value(
                $this->app()->make(ProvidesResolver::class),
            );
    ...
    public function testGetArgKey(
        ...
        if (count($directiveNode->arguments) === 0) {
            $directive
                ->shouldUseProperty('streamType')
                ->value(
                    $this->app()->make(StreamType::class),
